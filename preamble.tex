...
% \pujainfobox[image-path]{body} -- a Wikipedia-style summary panel for the
% top of a puja chapter. The image is optional; omit the optional argument
% (or leave it empty) to get a text-only box. #2 is free-form content, so
% each puja can list whichever facts (deity, occasion, month, source text,
% ...) actually apply to it.
\newcommand{\pujainfobox}[2][]{%
\begin{center}
\begin{tcolorbox}[
...
\ifblank{#1}{}{\begin{center}\includegraphics[width=0.34\linewidth]{#1}\end{center}\medskip}
#2
\end{tcolorbox}
\end{center}
}

% \pujanote{text} -- a slim callout for procedural stage-directions
% (e.g. "having done X, now do Y"), set apart from the mantras/shlokas.
